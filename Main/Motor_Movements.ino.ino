...
bool align()
{
  ...
  if (SIDE_2_READING > out_range_value) { 
    //robot rotate until at least side sensor 2 (long range) obtains a value
  ...
  //the robot not align with the central line or incorrect orientation
  if (abs(sideTarget - SIDE_1_READING) < 5 || abs(SIDE_1_READING - SIDE_2_READING > 20) { 

    //too close to the wall aka need to move right to the central line while correcting the orientation if necessary
    if ((sideTarget - SIDE_1_READING) < 5) {  
       left_font_motor.writeMicroseconds(1500 + speed_val - side_orientation_correction + side_distance_correction);
       left_rear_motor.writeMicroseconds(1500 - speed_val - side_orientation_correction - side_distance_correction);
       right_rear_motor.writeMicroseconds(1500 - speed_val - side_orientation_correction - side_distance_correction);
       right_font_motor.writeMicroseconds(1500 + speed_val - side_orientation_correction + side_distance_correction);
    } 
    //too far from the wall aka need to move left to the central line while correcting the orientation if necessary
    else if ((sideTarget - SIDE_1_READING) > 5) {  
      left_font_motor.writeMicroseconds(1500 - speed_val - side_orientation_correction - side_distance_correction);
      left_rear_motor.writeMicroseconds(1500 + speed_val - side_orientation_correction + side_distance_correction);
      right_rear_motor.writeMicroseconds(1500 + speed_val - side_orientation_correction + side_distance_correction);
      right_font_motor.writeMicroseconds(1500 - speed_val - side_orientation_correction - side_distance_correction);
  ...
  if (abs(SIDE_1_READING - SIDE_2_READING) < 20 && abs(sideTarget - SIDE_1_READING) < 5) {
...
//when robot reaches corner and stopped it uses gryro to turn exactly 90 degree
int cw ()
{
